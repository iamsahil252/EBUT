...
\]
by a uniformly random element of $\mathbb{G}_1$, while leaving the simulated proof machinery from $\mathsf{H}_2$ unchanged. Distinguishing $\mathsf{H}_3$ from $\mathsf{H}_2$ exactly distinguishes a Diffie--Hellman product from random in $\mathbb{G}_1$: given $(g_1,g_1^a,g_1^b,C)$, embed the two challenge clients' master secrets as $a$ and $b$, set $H_T=g_1$, and program the challenge nullifier as $N_T=C$. If $C=g_1^{ab}$ the transcript is real; if $C$ is random, the transcript is distributed as in $\mathsf{H}_3$. Therefore
\[
...
Take any accepted refresh tuple $(N_T,\pi_{\mathsf{refresh}},\mathcal{T}_{\mathsf{daily}})$ output by the adversary. By Lemma~\ref{lem:refresh-extractor}, except with the already-accounted-for outer knowledge error and BEQ failure probability, extraction yields a master secret $k_{\mathsf{sub}}$ satisfying
\[
N_T = k_{\mathsf{sub}} H_T.
\]
Therefore, for a fixed epoch $T$ and fixed master secret $k_{\mathsf{sub}}$, the nullifier is uniquely determined. If two valid refresh tuples for the same epoch came from the same master secret, they would necessarily contain the same nullifier, contradicting the game's requirement that the successful adversary output distinct nullifiers. Thus
\[
\Pr[\text{Case 1}] \le \advantage{DL}(\mathcal{B}_{\text{DL}}^{(1)}) + \frac{q_{\mathsf{refresh}}}{q} + \frac{1}{2^{128}} + \negl(\lambda).
\]
